#!/usr/bin/env python3
"""
Paper Trading Engine — Bollinger Band Scalping Strategy (1h)
=============================================================
Runs the EXACT same signal logic as backtest_boll_scalp_1h.py against live 1h candles.
No real money involved.

Strategy:
  - Entry: close crosses BB lower/upper AND close is above/below EMA (trend filter)
  - SL   : entry ± SL_TP_RATIO × |bb_mid - entry|
  - TP1  : bb_mid (50% position closed, SL moved to breakeven)
  - TP2  : opposite band (remaining 50%)
  - Timeout: MAX_HOLD_BARS bars

Coins tracked: ETH, SOL, HYPE, SUI, BTC

Usage:
    python paper_trade_boll_scalp_1h.py           # normal start
    python paper_trade_boll_scalp_1h.py --reset   # wipe state and start fresh
"""
...
WARMUP_1H        = 300          # 1h bars for indicator warmup
SLEEP_BUFFER_SEC = 10           # seconds after candle close before processing

STATE_FILE       = _HERE / "paper_state_boll_scalp_1h.json"
TRADE_LOG_FILE   = _HERE / "paper_trades_boll_scalp_1h.csv"
BEST_PARAMS_FILE = _ROOT / "results/boll_scalp_1h/best_params.json"

API_KEY    = os.getenv("BINANCE_API_KEY", "")
API_SECRET = os.getenv("BINANCE_API_SECRET", "")

COINS = bs.COINS   # includes BTC
...
def fetch_ohlcv(ex, symbol: str, limit: int) -> pd.DataFrame:
    ...
    now_1h = pd.Timestamp.now(tz="UTC").floor("1h")
    df = df[df.index < now_1h]
...
def process_bar(cs: dict, row: pd.Series, params: dict, coin: str, ts) -> list:
    """Process one just-closed 1h candle. Mutates cs in place. Returns fill records."""
    records  = []
    cap      = cs["capital"]
...
def print_report(state: dict):
    ...
    print(f"\n{'═'*65}")
    print(f"  PAPER PORTFOLIO (Boll Scalp 1h)  |  {now}")
    print(f"{'═'*65}")
...
def run_cycle(ex, state: dict, best: dict):
    ...
    for symbol, coin in COINS:
        ...
        try:
            ...
            df  = bs.prepare(fetch_ohlcv(ex, symbol, WARMUP_1H))
            cs  = state[coin]
...
# ── Intrabar TP/SL monitor ────────────────────────────────────────────────────
INTRABAR_CHECK_INTERVAL = 60  # seconds (1h candle, check every 60s)

def check_intrabar_sl(ex, state: dict, best: dict):
    """Check forming 1h candle high/low against TP/SL for all open positions."""
    changed = False
    for symbol, coin in COINS:
        cs = state[coin]
...
def seconds_to_next_candle() -> float:
    ...
    return max((next_1h - now).total_seconds() + SLEEP_BUFFER_SEC, 0)
...
def main():
    ...
    print("╔═══════════════════════════════════════════════════════════╗")
    print("║   PAPER TRADING ENGINE — Boll Scalp (1h)                  ║")
    ...
    if not BEST_PARAMS_FILE.exists():
        print(f"ERROR: {BEST_PARAMS_FILE} not found. Run tune.py --strategy boll_scalp_1h first.")
        sys.exit(1)
