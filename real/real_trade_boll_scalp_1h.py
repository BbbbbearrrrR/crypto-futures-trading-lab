#!/usr/bin/env python3
"""
Live Trading Engine — Bollinger Band Scalping Strategy (1h)
============================================================
Identical logic to live_trade_boll_scalp.py, but runs on 1h candles.
...
Usage:
    python live/live_trade_boll_scalp_1h.py           # start
    python live/live_trade_boll_scalp_1h.py --reset   # wipe state and restart
    python live/live_trade_boll_scalp_1h.py --dry-run # signals only, no orders
"""
...
def sync_position(ex: ccxt.binanceusdm, cs: dict, symbol: str, coin: str) -> bool:
    ...
    if not cs["partial_done"] and contracts > 0:
        if contracts < cs["qty_full"] * 0.7 and contracts >= cs["qty_half"] * 0.5:
            tp1 = cs["tp1_price"]
            ep  = cs["entry_price"]
            d   = cs["direction"]
            ...
            rec = dict(
                ...
                qty         = cs["qty_half"], exit_reason="TP1",
                pnl_usdt    = round(pnl_tp1, 4), capital=round(cs["capital"], 4),
            )
    ...
    if contracts == 0:
        ...
        try:
            ...
            if closing:
                ...
                tp2_dist = abs(exit_price - cs["tp2_price"])
                tp1_dist = abs(exit_price - cs["tp1_price"])
                if cs["partial_done"]:
                    exit_reason = "TP2" if tp2_dist < sl_dist else "SL"
                else:
                    if tp1_dist < sl_dist and tp1_dist < tp2_dist:
        ...
        rec = dict(
            ...
            pnl_usdt    = round(pnl_close, 4), capital=round(cs["capital"], 4),
        )
        cs["trades"].append(rec)
        _log_trade(rec)
        ...
        cs.update({"in_trade": False, "partial_done": False, "open_time": None,
...
def print_report(state: dict):
    ...
    print(f"\n{'═'*65}")
    print(f"  LIVE PORTFOLIO (Boll Scalp 1h)  |  {now}{'  [DRY-RUN]' if DRY_RUN else ''}")
    print(f"{'═'*65}")
...
def run_cycle(ex: ccxt.binanceusdm, state: dict, best: dict):
    ...
    for symbol, coin in COINS:
        ...
        try:
            ...
            df  = bs.prepare(fetch_ohlcv(symbol, WARMUP_1H))
            cs  = state[coin]
...
def main():
    ...
    mode_str = "DRY-RUN" if DRY_RUN else "⚠ REAL MONEY ⚠"
    print("╔═══════════════════════════════════════════════════════════╗")
    print("║   REAL TRADING ENGINE — Boll Scalp 1h                    ║")
    ...
    if not DRY_RUN:
        confirm = input("⚠  REAL MONEY MODE. Type 'yes' to continue: ")
